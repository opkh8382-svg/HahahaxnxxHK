...
def set_user_persistent_state(user_id, key, value):
    db = load_data()
    user_id = str(user_id)
...
def get_user_persistent_state(user_id, key, default=None):
    db = load_data()
    user_id = str(user_id)
    if "users" in db and user_id in db["users"]:
        return db["users"][user_id].get(key, default)
    return default

# ================= Fancy Font Generator =================
FONT_MAPS = {
    "serif_bold": "𝐀𝐁𝐂𝐃𝐄𝐅𝐆𝐇𝐈𝐉𝐊𝐋𝐌𝐍𝐎𝐏𝐐𝐑𝐒𝐓𝐔𝐕𝐖𝐗𝐘𝐙𝐚𝐛𝐜𝐝𝚎𝚏𝚐𝚑𝚒𝚓𝚔𝚕𝚖𝚗𝚘𝚙𝐪𝐫𝚜𝚝𝚞𝚟𝚠𝚡𝚢𝚣𝟎𝟏𝟐𝟑𝟒𝟓𝟔𝟕𝟖𝟗",
    "italic": "𝘈𝘉𝘊𝘋𝘌𝘍𝘎𝘏𝘐𝘑𝘒𝘓𝘔𝘕𝘖𝘗𝘘𝘙𝘚𝘛𝘜𝘝𝘞𝘟𝘠𝘡𝘢𝘣𝘤𝘥𝘦𝘧𝘨𝘩𝘪𝚓𝚔𝚕𝚖𝚗𝘰𝘱𝘲𝘳𝘴𝘵𝘶𝘷𝘸𝘹𝘺𝘻0123456789",
    "bold": "𝗔𝗕𝗖𝗗𝗘𝗙𝗚𝗛𝗜𝗝𝗞𝗟𝗠𝗡𝗢𝗣𝗤𝗥𝗦𝗧𝗨𝗩𝗪𝗫𝗬𝗭𝗮𝗯𝗰𝗱𝗲𝗳𝗴𝗵𝗶𝚓𝗸𝚕𝚖𝗻𝗼𝗽𝗾𝗿𝘀𝘁𝘂𝘃𝘄𝗅𝗾𝗿𝘀𝘁𝘂𝚟𝘄𝘅𝘆𝘇𝟬𝟭𝟮𝟯𝟰𝟱𝟲𝟳𝟴𝟵",
    "fullwidth": "ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺａｂｃｄｅｆｇｈｉｊｋ𝚕𝚖ｎｏｐｑｒ𝚜ｔｕｖｗｘｙｚ０１２３４５６７８９",
    "circles": "ⒶⒷⒸⒹⒺⒻⒼⒽⒾⒿⓀⓁⓂⓃⓄⓅⓆⓇⓈⓉⓊⓋⓌⓍⓎⓏⓐⓑⓒⓓⓔⓕⓖⓗⓘⓙⓚⓛⓜⓝⓞⓟⓠⓡⓢⓣⓤⓥⓦⓧⓨⓩ⓪①②③④⑤⑥⑦⑧⑨",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        set_user_persistent_state(user_id, "mode", None)
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.message.from_user
    if user.username == "vskmm2":
        db = load_data()
        total_users = len(db.get("users", {}))
        await update.message.reply_text(f"📊 **ស្ថិតិអ្នកប្រើប្រាស់ Bot សរុប៖**\n👥 ចំនួនអ្នកប្រើប្រាស់៖ **{total_users}** នាក់", parse_mode="Markdown")
    else:
        await update.message.reply_text("❌ អ្នកគ្មានសិទ្ធិប្រើប្រាស់คำสั่งនេះទេ។")
...
async def removebg_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    set_user_persistent_state(update.message.from_user.id, "mode", "remove_bg")
...
async def boost_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    set_user_persistent_state(update.message.from_user.id, "mode", None)
    boost_kb = [
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("copy_f_"):
        ...
        set_user_persistent_state(user_id, "mode", "vpn_enter_name")
        ...
        set_user_persistent_state(user_id, "tts_mode", data)
        set_user_persistent_state(user_id, "mode", "tts")
        mode_name = data.replace("tts_", "").replace("_", " ")
        await query.message.reply_text(f"✅ បានកំណត់សំឡេង៖ **{mode_name}**\n\nឥឡូវនេះអ្នកអាចផ្ញើសារ ឬអក្សរជាបន្តបន្ទាប់ដើម្បីបម្លែងជា Voice Note បានភ្លាមៗដោយសេរី!", parse_mode="Markdown")

    elif data.startswith("set_tr_"):
        mode_val = data.replace("set_tr_", "")
        set_user_persistent_state(user_id, "trans_mode", mode_val)
        set_user_persistent_state(user_id, "mode", "translate")
        await query.message.reply_text("✅ បានកំណត់ទិសដៅបកប្រែជោគជ័យ!\n\nឥឡូវនេះអ្នកអាចផ្ញើសារ/អក្សរជាបន្តបន្ទាប់ដើម្បីបកប្រែបានភ្លាមៗ។")

# ================= Photo Handler =================
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    current_mode = get_user_persistent_state(user_id, "mode")
    
    if current_mode == 'remove_bg':
        ...
        input_path = f"downloads/{user_id}_{int(time.time())}.jpg"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📥 ទាញយកវីដេអូ/MP3":
        set_user_persistent_state(user_id, "mode", "download")
        await context.bot.send_chat_action(chat_id=update.message.chat_id, action=constants.ChatAction.TYPING)
        ...
        set_user_persistent_state(user_id, "mode", "font_style")
        ...
        set_user_persistent_state(user_id, "mode", "remove_bg")
        await context.bot.send_chat_action(chat_id=update.message.chat_id, action=constants.ChatAction.TYPING)
        await update.message.reply_text("✂️ **ទម្រង់លុបផ្ទៃខាងក្រោយ៖**\nសូមផ្ញើរូបភាពរបស់អ្នកមកទីនេះ Bot នឹងលុប BG ជូនជាទម្រង់ PNG ថ្លា!", parse_mode="Markdown")
        return
        ...
        set_user_persistent_state(user_id, "mode", None)
        await context.bot.send_chat_action(chat_id=update.message.chat_id, action=constants.ChatAction.TYPING)
        current_keys = register_or_update_user(user_id)
    ...
    mode = get_user_persistent_state(user_id, "mode")
    ...
    # --- 5. ដំណើរការ TTS (Persistent Mode) ---
    if mode == 'tts':
        tts_mode = get_user_persistent_state(user_id, "tts_mode", "tts_Loveណាចា")
    ...
    # --- 6. ដំណើរការ Translation (Persistent Mode) ---
    if mode == 'translate':
        trans_mode = get_user_persistent_state(user_id, "trans_mode", "km_en")
